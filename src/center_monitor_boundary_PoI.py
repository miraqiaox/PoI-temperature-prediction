...
parser.add_argument('--str_root', default='/mnt/share1/liqiao/EAAI_dis/structure/400/center_monitoring_boundary_PoI', type=str, help='struc mat root')
...
y_point = np.zeros((400, 2))
for i in range(100):
    y_point[i] = [i * 2, 1]
    y_point[i + 100] = [199, i * 2]
    y_point[i + 200] = [(i + 1) * 2 - 2, 199]
    y_point[i + 300] = [1, (i + 1) * 2 - 1]
y_point = np.array(y_point, dtype=int)
...
# 读取数据
print("load data")
trainset, testset, point, x_std = load_data(args.data_root, x_point, y_point, 10000, args.rate, 'cpu')
...
for epoch in range(args.epochs):
    ...
    if train_loss < mae:
        torch.save(model.state_dict(),
                    "/mnt/share1/liqiao/EAAI_VB/graph_gcn/model_pkl/center_monitoring_boundary_PoI/net_hidf{:d}_nhid{:d}_bs{:d}_{:d}.pth".format(
